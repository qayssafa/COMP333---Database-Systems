...
public class ExecutionCasesController {
	private Stage stage;
	private Scene scene;
	private Parent root;
	private ArrayList<Execution_cases> data;
	private ObservableList<Execution_cases> dataList;
	...
	@FXML
	private TableColumn<Execution_cases , Integer> Case_ID;
	@FXML
	private TableColumn<Execution_cases , String> Case_description;
	@FXML
...
//----------------------------------------------------------------------
	...
	public void add(ActionEvent event) {
		try {
			Execution_cases rc;
			rc = new Execution_cases(Integer.parseInt(case_id_A.getText()),Case_description_A.getText());
			dataList.add(rc);
			insertData(rc);		
			case_id_A.clear();
...
//----------------------------------------------------------------------
	...
	private void insertData(Execution_cases rc) {
		try {
			Connector.a.connectDB();
			String sql = "Insert into Execution_cases (Case_id, case_description) values(?,?)";
...
//----------------------------------------------------------------------
	...
	public void getData() {
		String SQL = "select * from Execution_cases order by Case_id asc";
		try {
			Connector.a.connectDB();
			java.sql.Statement state = Connector.a.connectDB().createStatement();
			ResultSet rs = state.executeQuery(SQL);
			while (rs.next()) {
				Execution_cases it = new Execution_cases(rs.getInt(1), rs.getString(2));
				dataList.add(it);
			}
			rs.close();
			state.close();
...
//----------------------------------------------------------------------
	...
	public void updatecaseID(int old_id,int Date_ID_U) {
		try {
			Connector.a.connectDB();
			Connector.a.ExecuteStatement("update Execution_cases set Case_id = " + Date_ID_U + " where Case_id = " + old_id + ";");
			Connector.a.connectDB().close();
		} catch (SQLException e) {
			Message.displayMassage("Please confirm the values. Prevents repetition and false values","Warning");
		} catch (ClassNotFoundException e) {
...
//----------------------------------------------------------------------
	...
	public void updatecasedescription(int old_id , String case_description ) {
		try {
			Connector.a.connectDB();
			Connector.a.ExecuteStatement("update Execution_cases set case_description = '" + case_description + "' where Case_id = " + old_id + ";");
			Connector.a.connectDB().close();
...
//----------------------------------------------------------------------
	...
	private void deleteRow(int Case_id) {
		try {
			Connector.a.connectDB();
			Connector.a.ExecuteStatement("delete from  Execution_cases where Case_id =" + Case_id);
			Connector.a.connectDB().close();
...
//----------------------------------------------------------------------
	...
	public void searchItems() {
		FilteredList<Execution_cases> filteredData = new FilteredList<>(dataList, b -> true);
		...
		SortedList<Execution_cases> sortedData = new SortedList<>(filteredData);
...
//----------------------------------------------------------------------
	...
	public void initialize() {
		...
		Case_ID.setCellValueFactory(new PropertyValueFactory<Execution_cases, Integer>("Case_id"));
		Case_ID.setCellFactory(TextFieldTableCell.<Execution_cases, Integer>forTableColumn(new IntegerStringConverter()));
		Case_ID.setOnEditCommit((CellEditEvent<Execution_cases, Integer> t) -> {
		((Execution_cases) t.getTableView().getItems().get(t.getTablePosition().getRow())).setCase_id(t.getNewValue()); 
		updatecaseID(t.getRowValue().getCase_id(), t.getNewValue());
		});
			
			
		Case_description.setCellValueFactory(new PropertyValueFactory<Execution_cases, String>("case_description"));
		Case_description.setCellFactory(TextFieldTableCell.<Execution_cases>forTableColumn());
		Case_description.setOnEditCommit((CellEditEvent<Execution_cases, String> t) -> {
		((Execution_cases) t.getTableView().getItems().get(t.getTablePosition().getRow())).setCase_description(t.getNewValue());
